...
void quiz::MakeQuiz( int qznum )
{
	...
	/* fill in general questions*/
		...
		qzModel.setQuery( QString( "SELECT qnum FROM Quiz WHERE quiznum=%1 AND section=%2 ORDER BY qnum" )
					.arg( qznum ).arg( sec ) );
		if( qzModel.lastError().isValid() )
		{
			pwin->Err( qzModel.lastError().text() );
		...
		for( i=qnum=0; qnum<qCount[sec]; qnum++ )
		{     
			if( (i < qzModel.rowCount()) && (qnum == qzModel.data(qzModel.index(i, 0)).toInt()) )
				i++;
			else
				rnum = ChooseQuest( &qmodel, qznum, sec, qnum, qzquestrows, 0 );
		}
	}
}
...
int quiz::GetQnum( const int qznum, const int sec, const int key, const int qmin, const int qmax )
{
	...
	for( j=0; j<50; j++ )
	{
		...
		qzModel.setQuery( QString( "SELECT qnum FROM Quiz WHERE quiznum=%1 AND section=%2 AND qnum=%3" )
		...
		if( qzModel.rowCount() == 0 )
		{
			qzModel.setQuery( QString( "SELECT qnum FROM Quiz WHERE "
						"quiznum=%1 AND section=%2 AND qnum>%3 AND qnum<%4 AND key=%5" )
						.arg( qznum ).arg( sec ).arg( qnum-4 ).arg( qnum+4 ).arg( key ) );
			if( qzModel.lastError().isValid() )
			{
				pwin->Err( qzModel.lastError().text() );
				return -1;
	...
	if( j==50 )
		pwin->Err( QString( "Unable to find legal question spacing; quiz %1 section %2 key %3 question %4.")
			.arg(qznum+1).arg(sec+1).arg(key).arg(qnum+1) );
